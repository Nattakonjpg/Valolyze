...
def sage():
    return render_template('SageTest.html')
...
def clear_folders():
    ...
    for folder in folders_to_clear:
        try:
            # รับรายการไฟล์ในโฟลเดอร์
            files = os.listdir(folder)
            # ลบไฟล์ทั้งหมดในโฟลเดอร์
            for file in files:
                file_path = os.path.join(folder, file)
                os.remove(file_path)
            print(f'Files in folder {folder} cleared successfully!')
        except FileNotFoundError:
            print(f'Folder {folder} does not exist!')
        except Exception as e:
            print(f'Failed to clear files in folder {folder}: {e}')

    return jsonify({'message': 'Files in folders cleared successfully'})
...
def upload_file1():
    if request.method == 'POST':
        ...
        if file:
            ...
            # เรียกใช้หลัง main.py หลังอัพโหลดเสร็จ
            subprocess.Popen(
                ["python", "/home/nattakonpu/codes/Valolyze/Backend/main1.py"])
